...
class RecipeFilterTest {
    ...
    @ParameterizedTest
    @MethodSource("prepareRecipeRequest")
    void testFindAllByRecipeFilter(List<Recipe> expected, RecipeFilterRequest request) {
        final List<Recipe> all = repo.findAll(service.makePredicate(request));
        assertEquals(expected.size(), all.size());
        assertEquals(expected, all);
    }

    private static Stream<Arguments> prepareRecipeRequest() {
        List<Arguments> argumentsList = new ArrayList<>();
        RecipeFilterRequest request = new RecipeFilterRequest();
        argumentsList.add(Arguments.of(recipes, request));
        request = new RecipeFilterRequest();
        ...
        request = new RecipeFilterRequest();
        final String ingrName = recipes.get(1).getIngredients().get(1).getName();
        request.setInput(ingrName);
        argumentsList.add(Arguments.of(Collections.singletonList(recipes.get(1)), request));
        request = new RecipeFilterRequest();
        request.setInput(ingrName);
        request.setIngredient(true);
        argumentsList.add(Arguments.of(Collections.singletonList(recipes.get(1)), request));
        request = new RecipeFilterRequest();
        request.setInput(ingrName);
        request.setIngredient(false);
        argumentsList.add(Arguments.of(Collections.emptyList(), request));

        request = new RecipeFilterRequest();
        request.setInput("sweet");
        argumentsList.add(Arguments.of(Collections.emptyList(), request));
        request = new RecipeFilterRequest();
        request.setInput("sweet");
        request.setCategory("swe");
        argumentsList.add(Arguments.of(Collections.emptyList(), request));
        request = new RecipeFilterRequest();
        request.setInput("tit");
        request.setCategory("swe");
        argumentsList.add(Arguments.of(recipes, request));
